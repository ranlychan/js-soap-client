/** ***************************************************************************\
 ...
 Forked from https://github.com/gotshub/js-soap-client  and improved by ranlychan.
 For new versions check: https://github.com/ranlychan/js-soap-client
 * Original work by Matteo Casati (based on v2.4 from 2007-12-21)
 * Improved by Gordon Tschirner (https://github.com/gtathub)
 * Another improved by Ranly Chan (https://github.com/ranlychan)
 * Licensed under GPLv2: https://github.com/gtathub/js-soap-client.git
 \*****************************************************************************/

export function SOAPClientParameters() {
  var _pl = new Array()
  var _sl = new Array()
  ...
  switch (typeof (o)) {
    ...
    case 'object':
      ...
      // Object or custom function
        ...
        s += '<' + t + '>'
        for (var p in o) { s += SOAPClientParameters._serialize(p, o[p]) }
        s += '</' + t + '>'
      }
      break
    default:
...
SOAPClient.invoke = function(url, method, parameters, async, callback) {
  if (async) { SOAPClient._loadWsdl(url, method, parameters, async, callback) } else { return SOAPClient._loadWsdl(url, method, parameters, async, callback) }
}
...
SOAPClient._loadWsdl = function(url, method, parameters, async, callback) {
  ...
  if (wsdl != null && typeof wsdl !== 'undefined' && wsdl != '' && wsdl != 'undefined') { return SOAPClient._sendSoapRequest(url, method, parameters, async, callback, wsdl) }
  // get wsdl
  ...
  if (async) {
    xmlHttp.onreadystatechange = function() {
      if (xmlHttp.readyState == 4) { SOAPClient._onLoadWsdl(url, method, parameters, async, callback, xmlHttp) }
    }
  }
  xmlHttp.send(null)
...
SOAPClient._sendSoapRequest = function(url, method, parameters, async, callback, wsdl) {
  ...
  if (async) {
    xmlHttp.onreadystatechange = function() {
      if (xmlHttp.readyState == 4) { SOAPClient._onSendSoapRequest(method, async, callback, wsdl, xmlHttp) }
    }
  }
  console.log(sr)
  xmlHttp.send(sr)
  ...
  if (nd.length == 0) {
    if (req.responseXML.getElementsByTagName('faultcode').length > 0) {
      if (async || callback) {
        o = new Error()
        o.code = 500
        o.message = req.responseXML.getElementsByTagName('faultstring')[0].childNodes[0].nodeValue
      } else {
        var e = new Error('request fail')
        e.code = 500
        e.message = req.responseXML.getElementsByTagName('faultstring')[0].childNodes[0].nodeValue
        throw e
      }
    }
  } else { o = SOAPClient._soapresult2object(nd[0], wsdl) }
  if (callback) { callback(o, req.responseXML) }
  if (!async) { return o }
}
...
SOAPClient._node2object = function(node, wsdlTypes) {
  ...
  // text node
  if (node.nodeType == 3 || node.nodeType == 4) { return SOAPClient._extractValue(node, wsdlTypes) }
  // leaf node
  if (node.childNodes.length == 1 && (node.childNodes[0].nodeType == 3 || node.childNodes[0].nodeType == 4)) { return SOAPClient._node2object(node.childNodes[0], wsdlTypes) }
  var isArray = false
  var tmpNodeNameObject = new Object()
  for (var i = 0; !isArray && i < node.childNodes.length; i++) {
    if (typeof tmpNodeNameObject[node.childNodes[i].nodeName] === 'undefined') { tmpNodeNameObject[node.childNodes[i].nodeName] = true } else isArray = true
  }
  var isarray = isArray || SOAPClient._getTypeFromWsdl(node.nodeName, wsdlTypes).toLowerCase().indexOf('arrayof') != -1
  // object node
  if (!isarray) {
    var obj = null
    if (node.hasChildNodes()) { obj = new Object() }
    for (var i = 0; i < node.childNodes.length; i++) {
      var p = SOAPClient._node2object(node.childNodes[i], wsdlTypes)

      // delete string like 'ns1:'
